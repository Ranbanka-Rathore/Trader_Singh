...
# Set event loop policy on Windows for psycopg compatibility
if os.name == 'nt':
...
async def check_and_backfill():
    print("🔍 Checking database for NIFTY candle data...")
    engine = get_db_engine()
    
    try:
        query = "SELECT count(*) FROM candles WHERE ticker = 'NIFTY' AND timeframe = '5m'"
        df = pd.read_sql(query, engine)
        count = df.iloc[0, 0]
        print(f"   ↳ Current 5m NIFTY candles in DB: {count}")
    except Exception as e:
        print(f"   ⚠️ Could not read candles table, initializing: {e}")
        count = 0

    if count < 500:
        print("🚜 Data insufficient (less than 500 candles). Triggering 30-day NIFTY backfill from Dhan...")
        try:
            with engine.connect() as conn:
                conn.execute("DELETE FROM candles WHERE ticker = 'NIFTY'")
                print("   ↳ Cleared old NIFTY records.")
        except Exception:
            pass
            
        success = await data_service.backfill_historical_data("NIFTY", days=30)
        if success:
            print("✅ 30-day NIFTY backfill from Dhan complete!")
        else:
            print("❌ Backfill failed. Please verify Dhan API status.")
            sys.exit(1)
    else:
        print("✅ Sufficient data found in local database.")
...
def run_backtest_strategy(df, strategy_mode, lot_size=25):
    """
    Simulates the exact trade entries and exits for a given strategy_mode:
    CREDIT_SPREAD, DEBIT_SPREAD, NAKED_OPTIONS, IRON_CONDOR, LONG_STRANGLE, CALENDAR_SPREAD, RATIO_SPREAD,
    LONG_STRADDLE, IRON_BUTTERFLY, DIAGONAL_SPREAD, DELTA_NEUTRAL
    """
    in_trade = False
    trade_bias = None
    entry_price = 0.0
    ...
    for i in range(50, len(df)):
        ...
        if in_trade:
            ...
            if strategy_mode == "IRON_CONDOR":
                ...
                is_sl = current_value >= net_premium * 2.0  # 100% loss (value doubled)
        ...
        if direction:
            ...
            if strategy_mode == "CREDIT_SPREAD":
                ...
                net_premium = entry_price * 0.01 # 1% of spot
            elif strategy_mode == "CALENDAR_SPREAD":
                net_premium = entry_price * 0.004 # 0.4% of spot
            elif strategy_mode == "RATIO_SPREAD":
                net_premium = 50.0 * 0.05 # strike width (50) * 0.05 = 2.5
            elif strategy_mode == "LONG_STRADDLE":
                net_premium = entry_price * 0.018 # 1.8% of spot
            elif strategy_mode == "IRON_BUTTERFLY":
                net_premium = 50.0 * 0.45 # strike width (50) * 0.45 = 22.5
            elif strategy_mode == "DIAGONAL_SPREAD":
                net_premium = entry_price * 0.008 # 0.8% of spot
            elif strategy_mode == "DELTA_NEUTRAL":
                net_premium = entry_price * 0.08 # 8.0% of spot
...
async def main():
    # 1. Ensure DB has NIFTY data
    await check_and_backfill()
    
    # 2. Load historical NIFTY candles
    df = load_data()
    if df.empty:
    ...
    # Find start and end date of the backtest
    start_date = df['IST_Time'].iloc[0].strftime("%Y-%m-%d")
    end_date = df['IST_Time'].iloc[-1].strftime("%Y-%m-%d")
    print(f"\n📅 Running backtests from {start_date} to {end_date} ({len(df)} 5-minute candles)...")
    
    # 4. Run strategies
    lot_size = 25
    
    print("\nSimulating Credit Spreads strategy...")
    credit_trades = run_backtest_strategy(df, "CREDIT_SPREAD", lot_size)
    
    print("Simulating Debit Spreads strategy...")
    debit_trades = run_backtest_strategy(df, "DEBIT_SPREAD", lot_size)
    
    print("Simulating Naked Options strategy...")
    naked_trades = run_backtest_strategy(df, "NAKED_OPTIONS", lot_size)
    
    print("Simulating Iron Condor strategy...")
    condor_trades = run_backtest_strategy(df, "IRON_CONDOR", lot_size)
    
    print("Simulating Long Strangle strategy...")
    strangle_trades = run_backtest_strategy(df, "LONG_STRANGLE", lot_size)
    
    print("Simulating Calendar Spread strategy...")
    calendar_trades = run_backtest_strategy(df, "CALENDAR_SPREAD", lot_size)
    
    print("Simulating Ratio Spread strategy...")
    ratio_trades = run_backtest_strategy(df, "RATIO_SPREAD", lot_size)

    print("Simulating Long Straddle strategy...")
    straddle_trades = run_backtest_strategy(df, "LONG_STRADDLE", lot_size)

    print("Simulating Iron Butterfly strategy...")
    butterfly_trades = run_backtest_strategy(df, "IRON_BUTTERFLY", lot_size)

    print("Simulating Diagonal Spread strategy...")
    diagonal_trades = run_backtest_strategy(df, "DIAGONAL_SPREAD", lot_size)

    print("Simulating Delta Neutral strategy...")
    neutral_trades = run_backtest_strategy(df, "DELTA_NEUTRAL", lot_size)
    
    # 5. Compile results
    results = [
        analyze_trades(credit_trades, "Credit Spreads (Theta Sell)"),
        analyze_trades(debit_trades, "Debit Spreads (Spread Buy)"),
        analyze_trades(naked_trades, "Naked Options (ATM Long Buy)"),
        analyze_trades(condor_trades, "Iron Condor (Delta Neutral Sell)"),
        analyze_trades(strangle_trades, "Long Strangle (OTM Buy Wings)"),
        analyze_trades(calendar_trades, "Calendar Spread (Weekly/Monthly)"),
        analyze_trades(ratio_trades, "Ratio Spread (OTM Credit Ratio)"),
        analyze_trades(straddle_trades, "Long Straddle (ATM Long Buy)"),
        analyze_trades(butterfly_trades, "Iron Butterfly (ATM Credit Fly)"),
        analyze_trades(diagonal_trades, "Diagonal Spread (Weekly/Monthly)"),
        analyze_trades(neutral_trades, "Delta Neutral (ATM Short Straddle)")
    ]
    
    df_results = pd.DataFrame(results)
    print("\n==========================================================================")
    print("📊 BACKTEST PERFORMANCE SUMMARY COMPARISON (NIFTY - 1 Lot)")
    print("==========================================================================")
